...
router.get("/software", requireClient, async (req, res) => {
  try {
    ...
    console.error("Erreur GET /software :", err);
...
router.get("/software/:licenseId/download", requireClient, async (req, res) => {
  try {
    ...
    if (isCloudinaryRef(version.fichier)) {
      ...
      const finalFilename = `okim-art-${safeTitre}${versionLabel}${ext}`;

      try {
        const url = await signedPrivateUrl(version.fichier, finalFilename, {
          onRepair: (repairedRef) => db.prepare("UPDATE software_versions SET fichier = ? WHERE id = ?").run(repairedRef, version.id)
        });
        return res.redirect(url);
      } catch (e) {
        console.error("Erreur signature URL logiciel Cloudinary :", e);
        return res.status(404).json({ error: e.message || "Ce fichier n'est plus disponible sur le stockage distant." });
      }
    }

    // CAS B : Stockage local
    const fullPath = path.join(PERSIST_ROOT, version.fichier);

    if (!fs.existsSync(fullPath)) {
      console.error(`Fichier logiciel local introuvable : ${fullPath}`);
      return res.status(404).json({ error: "Ce fichier n'est plus disponible sur le serveur local." });
    }
    ...
    return res.download(fullPath, finalFilename, (err) => {
      if (err && !res.headersSent) {
        console.error("Erreur lors de l'envoi du logiciel local :", err);
        res.status(500).json({ error: "Erreur lors du transfert du fichier." });
      }
    });

  } catch (err) {
    console.error("Erreur GET /software/:licenseId/download :", err);
    if (!res.headersSent) {
      res.status(500).json({ error: "Erreur serveur lors du téléchargement du logiciel." });
    }
  }
});
